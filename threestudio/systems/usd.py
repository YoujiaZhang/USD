...
@threestudio.register("usd-system")
class USD(BaseLift3DSystem):
    @dataclass
    ...
    def training_step(self, batch, batch_idx):
        ...
        GT_rays_o = batch['GT_rays_o']
        GT_rays_d = batch['GT_rays_d']
        GT_mvp_mtx = batch['GT_mvp_mtx']
        GT_proj_mtx = batch['GT_proj_mtx']
        batch['rays_o'] = GT_rays_o
        batch['rays_d'] = GT_rays_d
        batch['mvp_mtx'] = GT_mvp_mtx
        batch['proj_mtx'] = GT_proj_mtx
        GT_out = self(batch)
        ...
        if self.cfg.stage == "geometry":
            ...
            guidance_out = self.guidance(
                guidance_inp,
                guidance_inp,
                self.prompt_utils, **batch, rgb_as_latents=False
            )
        else:
            guidance_inp = out["comp_rgb"]
            guidance_inp_GT = GT_out["comp_rgb"]
            guidance_out = self.guidance(
                guidance_inp, 
                guidance_inp_GT,
                self.prompt_utils, 
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(out["comp_rgb"][0].shape)
        # print(out["opacity"][0].shape)
        if "comp_rgb" in out:
            img_rgba = torch.cat([out["comp_rgb"][0], out["opacity"][0]], dim=-1)
            save_image(img_rgba.float().permute(2, 0, 1), "getmesh/"+str(batch['index'][0].item())+".png")
